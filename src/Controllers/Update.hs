...
updateScene Play dt = checkGameOver 
 ...
 where
  ...
  spawnFruit dt gs = if allowed then randomSpawnFruit (floor $ dt * fromIntegral (s - 500)) gs else gs
    where 
      s = (unScore . unPacMan) gs
      fruits = [c | y <- getGridItems $ unLevel gs, c@(Collectible _ Fruit _) <- y]
      allowed = s > 500 && (all isCollected fruits) && length fruits < 2

  randomSpawnFruit chance gs = if r <= chance then randomSpawnFruitLocation gs' else gs'
